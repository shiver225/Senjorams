...
class _SleepScreenState extends State<SleepScreen> {
  ...
  TimeOfDay _selectedTimeWakeUp = TimeOfDay(
      hour: (TimeOfDay.now().hour + 8) % 24, minute: TimeOfDay.now().minute);
  ...
  void _loadData() {
    ...
    if (saved != null) {
      ...
      _alarmList = decoded
          .map((alarm) => Alarm.fromJson(Map<String, dynamic>.from(alarm)))
          .toList();
    }
  }
                ...
                children: <Widget>[
                  ...
                  Visibility(
                    ...
                    child: _toggleButtons2B(
                        ...
                        buttonHeight:
                            (MediaQuery.of(context).size.width - 36) / 10),
                  ),
                  const SizedBox(height: 20),
  ...
  Card _alarmCard(int index) {
    return Card(
      ...
      child: Column(
        ...
        children: <Widget>[
          InkWell(
            onTap: _alarmTrashCan.isEmpty
                ? () => _modalScrollPicker(context: context, index: index)
            ...
            child: ListTile(
              ...
              title: Text(
                _alarmList[index].linkedAlarmAmm == 0
                    ? _alarmList[index].time[0].format(context)
              ...
              subtitle: Text(
                _alarmList[index].linkedAlarmAmm == 0
                    ? "Vieną kartą"
                    : "Kasdienis | ${_alarmList[index].time[0].format(context)}",
                style: const TextStyle(
                  color: Colors.black,
                ),
              ),
              trailing: _alarmTrashCan.isEmpty
                  ? Switch(
        ...
        children: List.generate(isSelected.length, (index) {
          ...
          return InkWell(
            ...
            splashColor: const Color.fromARGB(
                255, 204, 177, 130), //the default splashColor is grey
            onTap: () {
            ...
            child: Ink(
              decoration: BoxDecoration(
                color: isSelected[index]
                    ? const Color.fromARGB(255, 221, 195, 149)
                    : Colors.white,
                borderRadius: BorderRadius.circular(20),
              ),
              child: Center(
    ...
    return ToggleButtons(
      ...
      children: <Widget>[
        SizedBox(
          ...
          height:
              buttonHeight ?? (MediaQuery.of(context).size.height - 36) / 20,
          child: Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: <Widget>[
              ...
              actions: [
                IconButton(
                    onPressed: () async {
                      ...
                      _alarmList
                          .removeWhere((item) => _alarmTrashCan.contains(item));
                      //disable alarm
                      await _saveData();
                      setState(() {
                        _alarmTrashCan.clear();
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  _alarmList.sort((a, b) {
                    if (b.linkedAlarmAmm.compareTo(a.linkedAlarmAmm) > 0) {
                      return 1;
                    } else if (b.linkedAlarmAmm.compareTo(a.linkedAlarmAmm) ==
                        0) {
                      int compare(a, b, index) => (a.time[index].hour +
                              a.time[index].minute / 60)
                          .compareTo(
                              b.time[index].hour + b.time[index].minute / 60);
                      if (b.linkedAlarmAmm == 0) {
                        return compare(a, b, 0);
                      } else {
                        if (compare(b, a, 0) > 0) {
                          return 1;
                        } else if (compare(b, a, 0) == 0) {
